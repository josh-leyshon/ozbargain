...
export function makeVoteButtons({ votes }: VoteButtonsProps): {
  positiveVoteButton: React.JSX.Element;
  negativeVoteButton: React.JSX.Element;
} {
  return {
    positiveVoteButton: <PositiveVoteButton votes={votes.positive} onPress={submitVote} />,
    negativeVoteButton: <NegativeVoteButton votes={votes.negative} onPress={submitVote} />,
  };
}
...
export function PositiveVoteButton({
  votes,
  onPress,
}: { votes: VoteButtonsProps['votes']['positive'] } & OnPress): React.JSX.Element {
  const title = `👍 ${votes}`;
  const colour = getColourForVoteCount('positive', votes);
  return <Button title={title} color={colour} onPress={() => onPress('positive')} />;
}
...
export function NegativeVoteButton({
  votes,
  onPress,
}: { votes: VoteButtonsProps['votes']['negative'] } & OnPress): React.JSX.Element {
  const title = `👎 ${votes}`;
  const colour = getColourForVoteCount('negative', votes);
  return <Button title={title} color={colour} onPress={() => onPress('negative')} />;
}
